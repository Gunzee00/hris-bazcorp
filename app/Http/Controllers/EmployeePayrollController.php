    ...
    public function store(Request $request)
{
    ...
    $data['tax_type'] = $request->input('tax_type', null);
    $data['tax_number'] = $request->input('tax_number', null);
    $data['insurance_number'] = $request->input('insurance_number', null);
    $data['insurance'] = $request->input('insurance', null);   
    $data['allowance'] = $request->input('allowance', null);   
    $data['bpjs_ketenagakerjaan'] = $request->input('bpjs_ketenagakerjaan', null);   

    $data['allowance'] = $allowancesJson;
    $successStatus = $this->employee_payroll_repository->create($data);
    $this->log_repository->logActivity('Created employee payroll', $request->ip());
...
//         'effective_salary_date' => 'required|date',
//         'title' => 'required|string',
//         'tax_flag' => 'required|boolean',
...
//     try {
//         $data = $request->only([
//             'employee_id', 'effective_salary_date', 'title', 'tax_flag', 
//             'salary_received', 'basic_salary', 'bpjs_ketenagakerjaan',
//             'bpjs_kesehatan', 'insurance', 'insurance_number', 'tax_number', 
//             'tax_type', 'pay_frequency'
//         ]);
...
//         $allowances = $request->input('allowances', []);
//         $data['allowance'] = json_encode($allowances);
//         foreach ($allowances as $allowance) {
//             $data['total_allowance'] += $allowance['amount'] ?? 0;
//         }

//         $this->employee_payroll_repository->create($data);
//         $this->log_repository->logActivity('Created employee payroll', $request->ip());
    ...
    public function update(Request $request, $id)
{
    ...
    // Handle the tax_type field, set it to null if not provided
    $data['tax_type'] = $request->input('tax_type', null);
    $data['tax_number'] = $request->input('tax_number', null);
    $data['insurance_number'] = $request->input('insurance_number', null);
    $data['insurance'] = $request->input('insurance', null);
    ...
    $data['allowance'] = $allowancesJson;
    $successStatus = $this->employee_payroll_repository->update($id, $data);
    $this->log_repository->logActivity('Updated employee payroll with id: ' . $id, '', $request->ip());
